...
| Digitados do Dia | todos | `ShellRankingToday.jsx` | Digitados do dia por valor de referência + telão |
...
| **Digitados ordena por R$ de referência** | Decisão de 20/08/2026 — antes era por quantidade de contratos, que passou a desempatar. Quem digitou 40 contratos pequenos fica atrás de quem digitou 12 grandes. O número grande da linha virou o R$ nas **duas** variantes: número grande fora da ordem faz a lista parecer desordenada, e o `ri-unidade` ("digitados" em caixa alta ao lado do total) saiu junto, com a contagem descendo para a linha de apoio |
...
> O título continua "Digitados do Dia" — o **recorte** ainda é o que foi digitado no dia (`tipo: 'cadastro'`, pago ou não); só a **ordem** deixou de ser a contagem. O subtítulo da página diz "por valor de referência" para a diferença não ficar implícita.
...
| Ago/19 | Aba do navegador sem ícone: o `index.html` aponta para `/trophy.svg` desde sempre, mas `frontend/public/` **não existia** — o arquivo nunca foi criado. Em produção a URL respondia 200 com `text/html`, porque o fallback da SPA do Express devolve o `index.html` para caminho desconhecido; o navegador recebia HTML no lugar da imagem e caía no ícone padrão | Criado `frontend/public/trophy.svg` (o Vite copia `public/` para o `dist` no build). Desenhado para **16px**, que é o tamanho em que ele aparece: silhueta cheia, sem traço fino, e **fundo próprio** em vez de transparência — aba clara e aba escura são as duas possíveis, e um troféu recortado sumiria em uma delas. Ouro sobre verde escuro dá **5,7:1**; o mesmo ouro sobre o verde da interface (`#3DB879`) daria **1,4:1** e viraria mancha. Comentário XML **não pode conter `--`**: escrever o nome do token `--gd-ink` dentro do comentário fez o Chrome recusar o arquivo inteiro com `parsererror`, e o sintoma foi o mesmo ícone quebrado que se queria corrigir. A barra lateral continua com o logo do Grupo Digital — decisão do cliente |
...
| Ago/20 | "Digitados do Dia" ordenava por quantidade de contratos; o cliente quer o topo por **valor de referência** | `digitadosDoDia` passou a chamar `agregar({ ordenarPor: 'valor' })` — a mesma ordem do mensal, com a contagem no desempate (o comparador já existia e servia os dois casos). Na tela, a variante `digitados` da `Linha` deixou de mostrar a contagem como número grande: os dois rankings agora destacam o R$, que é o que ordena a lista nos dois. `.ri-unidade` ficou sem uso e saiu do `shell.css`. Sem a mudança na tela, a lista pareceria desordenada — número grande em destaque que não é o critério de ordenação |
