...
| Jun/30 | INDICACAO e CONTRATO_10K oscilavam a cada 5min | API NewCorban retornava `{}` (sem `error:true`) em rodadas alternadas → `proposalsOk=true` mas `rawProposals=[]` → cron deletava eventos. Fix: (1) guard `if (!isForce && rawProposals.length === 0) return []` antes do loop — aborta rodada cron com API vazia; (2) INDICACAO: `else` → `else if (isForce)` — cron nunca remove evento histórico acumulado |
